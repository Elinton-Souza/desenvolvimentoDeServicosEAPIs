...
router.post("/", async (req, res) => {
    ...
    if (!destino || !transporte || !preco || !dataSaida || !dataRetorno) {
        res.status(400).json({ erro: "Informe o destino, transporte, preço, dataSaida e dataRetorno" })
        return
    }
...
router.put("/:id", async (req, res) => {
    ...
    if (!destino || !transporte || !preco || !dataSaida || !dataRetorno || !roteiro) {
        res.status(400).json({ erro: "Informe o destino, transporte, preço, dataSaida, dataRetorno e roteiro" })
        return
    }
